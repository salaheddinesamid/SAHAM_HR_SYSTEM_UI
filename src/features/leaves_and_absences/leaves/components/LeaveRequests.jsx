...
const LeaveRequestsTable = ({requests, loading, currentPage, currentSize, setCurrentPage, handleChangeRowsPerPage, totalElements, onClickCancel})=>{
    return (
        <div className="row">
...
export const LeaveRequests = ({user, filters})=>{
    ...
    const fetchAbsenceRequests = async(page = 0, size = 8)=>{
        ...
        try{
            setLoading(true);
            const res = await getMyLeaveRequests(email, page, size);
